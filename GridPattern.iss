...
#define ApplicationName "GridPattern v1.2"

AppName    = GridPattern v1.2
AppVersion = 1.2
...
OutputBaseFilename = GridPattern v1.2
...
#define outFiles[0] "experts\GridPattern v1.2.mq4"
#define outFiles[1] "experts\GridPattern v1.2.ex4"
...
Source: "{#inFiles[4]}"; DestDir: "{tmp}"; Flags: dontcopy
Source: "{#inFiles[5]}"; DestDir: "{tmp}"; Flags: dontcopy
...
//TODO  Add or remove needed lines here 
    ...
    for i := 0 to GetArrayLength(InstalledTerminals) - 1 do begin
      if (InstalledTerminalsCheckBoxes.Checked[i]) then begin
        ...
        // Copy file #5
        if (not DirExists(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[4]}')))) then begin
          CreateDir(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[4]}')));
        end;
        if FileCopy(ExpandConstant('{tmp}') + '\' + GetFileFromPath(ExpandConstant('{#inFiles[4]}')), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[4]}'), false) then begin
          RegWriteStringValue(HKEY_LOCAL_MACHINE, ExpandConstant(UNINSTALL_SUBKEY + '\{#ApplicationName}'), 'InstallLocation_5_' + IntToStr(i), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[4]}'));
        end;   

        // Copy file #6
        if (not DirExists(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[5]}')))) then begin
          CreateDir(InstalledTerminals[i].Location + '\' + GetDirFromPath(ExpandConstant('{#outFiles[5]}')));
        end;
        if FileCopy(ExpandConstant('{tmp}') + '\' + GetFileFromPath(ExpandConstant('{#inFiles[5]}')), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[5]}'), false) then begin
          RegWriteStringValue(HKEY_LOCAL_MACHINE, ExpandConstant(UNINSTALL_SUBKEY + '\{#ApplicationName}'), 'InstallLocation_6_' + IntToStr(i), InstalledTerminals[i].Location + '\' + ExpandConstant('{#outFiles[5]}'));
        end;   
